...
eligible no-win opportunity > 0 かつ selected = 0 -> source contract violation
```

この2つを混同しない。24 hanchanでdaiminkan / ankan / kakanの3種類すべてを観測する
ことはqualificationの必須条件にしない。missing kan kindはfabricated coverageの理由に
ならない。
...
| Item | Value |
|---|---|
| generation `lisjong-arena` | `18297df5cf1ca4025b7a0a6e31b94a0a954109bf` |
...
本pilotのgenerationは`18297df`で実行した。それより後のcommitはREADME / 本書等の
documentationだけであり、generation path (`stage3_kan_coverage` / `stage3_entry_gate` /
Phase 2・4・5) はbyte-identicalである。
...
| `raw_corpus_identity` | `b78dbe8092f990173c069170e6d9ae3bf188b588c9651284420606d3a14cf909` |
| `dataset_identity` | `46aa37f41f19b34204e696f163105b98041b41ebb9578d077cfc63b299fd3fd4` |
| result schema | `stage3-kan-coverage-result-v1` |

artifactはrepository外に生成し、Gitへcommitしていない。

### Reproducibility

同じplan / 同じarena revisionによる独立再生成は実行中である（結果確定後に本節へ追記する）。

1回のgeneration内部では、次の2つのdeterminism checkが既に成立している。
...
| Kind | legal opportunities | with winning action also legal | eligible no-win | selected |
|---|---|---|---|---|
| daiminkan | 54 | 0 | 54 | 54 |
| ankan | 24 | 0 | 24 | 24 |
| kakan | 11 | 0 | 11 | 11 |
| **total** | **89** | **0** | **89** | **89** |

```text
selection contract violations   0
```
...
bindingでpublic evidenceへ対応付けられた。`unaccounted`は0であり、これは推測で
埋めた0ではなくbindingが成立した結果の0である。
...
| Metric | Value |
|---|---|
| wall-clock s / hanchan (generation全体) | 199.80 |
| CPU-s / hanchan (generation全体) | 199.77 |
| うちrecordingのみ (wall-clock s / hanchan) | 90.42 |
| CPU-s / anchor | 0.41007 |
...
| deterministic same-plan regeneration / canonical identity | Phase 2 equality re-runとPolicy pass equalityはPASS / 独立再生成は実行中 |
