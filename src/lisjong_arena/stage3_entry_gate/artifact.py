    ...
    *,
    ...
    return {
        ...
        "checkpoint_selection_rule": (
            "lowest pooled self-rollout VALIDATION per-tile MAE"
        ),
...
def validate_model_manifest(value: object) -> dict[str, object]:
    ...
    if selected not in epochs:
        raise Stage3ArtifactError("selected_epoch is outside loss_history")
    best = min(row["validation_mae"] for row in history)
    if history[selected - 1]["validation_mae"] != best:
        raise Stage3ArtifactError(
            "selected checkpoint differs from the lowest VALIDATION MAE rule"
        )
    return value
