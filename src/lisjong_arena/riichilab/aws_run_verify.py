...
def _parse_utc(value: str, field_name: str) -> datetime:
    try:
        ...
        raise AwsRunVerificationError(f"{field_name} is not valid ISO-8601 UTC") from exc
...
def _scan_paths(
    *,
    ...
    if not token:
        raise AwsRunVerificationError("runtime token is unavailable for exact-byte scan")
...
def verify_run(
    *,
    ...
    if runner["stopped reason"] != "duration_reached":
        raise AwsRunVerificationError("runner did not stop because duration was reached")
    if runner["requested duration seconds"] != str(expected_duration_seconds):
        raise AwsRunVerificationError("runner duration does not match expected duration")
    ...
    if elapsed_seconds + 1 < expected_duration_seconds:
        raise AwsRunVerificationError("elapsed runtime is shorter than requested duration")
