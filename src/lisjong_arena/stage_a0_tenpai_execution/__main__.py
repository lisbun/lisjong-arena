...
from .result import build_result, save_result
...
def execute(arguments: argparse.Namespace) -> int:
    ...
    if root.exists():
        raise FileExistsError(
            "scientific output root already exists; #262 does not permit an in-place rerun"
        )
    ...
    for seed in locked.TRAINING_SEEDS:
        ...
        for training_result in (a_result, t_result):
            ...
            checkpoint = save_checkpoint(destination, scientific, training_result)
    ...
    frozen = tuple(
        load_checkpoint(checkpoint.path)
    ...
    if gate_label == GATE_PASS:
        indexed = {(checkpoint.arm.value, checkpoint.seed): checkpoint for checkpoint in frozen}
