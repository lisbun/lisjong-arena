...
- Issue #53で、first-party `lisjong-engine`上でlisjong Policyを実行するArena-owned bridge(`lisjong_arena.engine`)を追加した。RiichiEnv execution pathと並ぶ2本目のconcrete execution pathであり、共通のbackend abstractionへは統合しない。`SeatObservation` -> `PolicyInput`のprojectionは、engine Issue #38のplayer-safe snapshotをそのまま利用し、consumer-side history materializerを導入しない。descriptorと`InternalAction`の対応は1 seat・1 decisionに閉じ、semantic duplicate collapseはfail closedする。現時点でこのpathはGameTraceを発行せず、AABB / ABBB evaluation protocolへも接続していない
- `lisjong` / `lisjong-engine` dependencyは再現可能性のためrelease tagが出るまでfull commit SHAへpinする。`lisjong`の現在のpinは`376f69088a134b5a9bcc33a69b95e3f779eb2b0e`である(Issue #45でGameTrace cleanup後revisionへ同期)。`lisjong-engine`の現在のpinは`7077e6da5e873c779ffe0c8c2626b2acf17ad273`である(engine Issue #38 / PR #39 merge後revision、Issue #53で追加)
