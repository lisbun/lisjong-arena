...
- RiichiEnv Adapterはcanonical physical implementation・lisjong legacy cleanup・Arena exact pin syncのすべてが完了し(上記)、pillarとしてCOMPLETEである。`GameTrace`はIssue #43でcanonical physical implementation(`lisjong_arena.game_trace`)へのArena takeoverを完了したが、lisjong legacy cleanupとexact pin syncはまだ完了していないため、pillarとしては引き続きcleanup pendingである
- `lisjong` dependencyは再現可能性のためrelease tagが出るまでfull commit SHAへpinする。現在のpinは`3505321b62e7a2be204cc555924b485a898c8f31`である(Issue #41でRiichiEnv Adapter cleanup後revisionへ同期)
